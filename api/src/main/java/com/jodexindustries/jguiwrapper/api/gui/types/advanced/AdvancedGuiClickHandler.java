...
public interface AdvancedGuiClickHandler<G extends SimpleGui<G> & AdvancedGui<G>> extends GuiHandler<GuiClickEvent, G> {

    @Override
    default void handle(@NotNull GuiClickEvent event, @NotNull G gui) {
        gui.getController(event.rawSlot()).ifPresent(controller -> this.handle(event, controller));
    }

    void handle(@NotNull GuiClickEvent event, @NotNull AdvancedGuiItemController<G, ?> controller);
}
